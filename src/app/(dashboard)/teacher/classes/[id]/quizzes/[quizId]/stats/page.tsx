...
import {
  ...
  DistributionBars,
  RateBar,
} from "@/components/teacher/stats-ui";
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            {stats.studentsAttempted} student
            {stats.studentsAttempted !== 1 ? "s" : ""} · {stats.attemptsTotal}{" "}
            attempt{stats.attemptsTotal !== 1 ? "s" : ""}
          </p>
        </div>
        <ExportGradesDialog
          classId={id}
      ...
      {!hasData ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <StatCard
              label="Pass rate"
              value={ratePct(stats.passRate)}
              sub="best ≥ 60%"
            />
            <StatCard
              label="Avg retakes"
              value={stats.avgAttemptsPerStudent.toFixed(1)}
              sub="per student"
            />
          </div>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="mt-3 text-xs text-muted-foreground">
                  Range {pct(stats.min)}–{pct(stats.max)} (best score per
                  student).
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <p className="mb-3 text-xs text-muted-foreground">
                  {simStats.uniqueStudents} student
                  {simStats.uniqueStudents !== 1 ? "s" : ""} opened a simulation
                  from their results, {simStats.totalSessions} session
                    ...
                    <tbody>
                      {simStats.engagement.map((row) => (
                        ...
                        >
                          ...
                          <td className="py-2 px-3 text-right tabular-nums">
                            {formatDurationMs(row.medianActiveMs)}
                          </td>
                          <td className="py-2 px-3 text-right tabular-nums">
                            {row.meanParamChanges.toFixed(1)}
                          </td>
                          <td className="py-2 px-3 text-right tabular-nums">
                            {ratePct(row.bounceRate)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  Bounce = sessions under 10s of activity or with no
                  interaction.
                </p>
              </CardContent>
            </Card>
      ...
      <Card>
        ...
        <CardContent>
          <p className="mb-3 text-xs text-muted-foreground">
            Manual percentages override calculated grades for this test in every
            export.
          </p>
          <ManualGradesTable
            classId={id}
