...
export default function MaterialPoolImportDialog({
  classId,
}: {
  classId: string;
}) {
  ...
  async function loadPool() {
    ...
    setSelected(new Set());
    try {
      const response = await fetch("/api/materials/pool", {
        cache: "no-store",
      });
      ...
      setError(
        cause instanceof Error
          ? cause.message
          : "Could not load the global pool.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function importSelected() {
    ...
    try {
      const response = await fetch(
        `/api/classes/${classId}/materials/pool/import`,
        {
          method: "POST",
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Browse materials by topic and import independent copies into this
            class.
          </DialogDescription>
        </DialogHeader>
        <div className="max-h-[55vh] space-y-5 overflow-y-auto px-1">
          {loading ? (
            ...
            <p className="py-10 text-center text-sm text-muted-foreground">
              The global material pool is empty.
            </p>
          ) : (
