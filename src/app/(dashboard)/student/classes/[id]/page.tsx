...
export default async function StudentClassPage({
  ...
  const groups = new Map<
    string,
    { topicName: string | null; quizzes: typeof quizzes }
  >();
      ...
      {quizzes.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center py-12 text-center">
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Your teacher hasn&apos;t published any quizzes yet. Check back
              soon!
            </p>
          </CardContent>
        </Card>
        ...
        <div className="space-y-6">
          {orderedGroups.map((group) => {
            const completed = group.quizzes.filter(
              (q) => progressMap.get(q.id)?.status === "COMPLETED",
            ).length;
                ...
                <CardContent>
                  <div className="space-y-2">
                    {group.quizzes.map((quiz, idx) => {
                      ...
                      const opensAt = settings?.availableFrom
                        ? new Date(settings.availableFrom)
                        : null;
                      const closesAt = settings?.availableUntil
                      ...
                      return (
                        ...
                        >
                          ...
                          {locked ? (
                            ...
                            >
                              <Link
                                href={`/student/classes/${id}/quiz/${quiz.id}`}
                              >
                                {status === "COMPLETED"
                                  ? "Retry"
                                  : status === "IN_PROGRESS"
