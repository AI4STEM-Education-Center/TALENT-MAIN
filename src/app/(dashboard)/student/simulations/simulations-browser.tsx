...
import type { StudentSimulation, StudentSimulationClass } from "@/lib/student-content";
 ...
 */
export function SimulationsBrowser({ classes }: { classes: StudentSimulationClass[] }) {
  const [classId, setClassId] = useState(ALL_CLASSES);
  const [activeId, setActiveId] = useState<string | null>(null);

  const visible = useMemo(
    () => (classId === ALL_CLASSES ? classes : classes.filter((c) => c.classId === classId)),
    [classes, classId]
  );
    ...
    <div className="space-y-6">
      {classes.length > 1 && (
        <div className="flex flex-wrap gap-2">
          {[{ classId: ALL_CLASSES, className: "All classes" }, ...classes].map((cls) => (
            <button
              key={cls.classId}
              type="button"
      ...
      {visible.map((cls) => (
        <section key={cls.classId} className="space-y-4" aria-label={cls.className}>
          ...
          {cls.quizzes.map((quiz) => (
            <div key={`${cls.classId}:${quiz.quizId}`} className="space-y-2">
              <h3 className="flex items-center gap-2 text-sm font-medium text-muted-foreground">
                ...
                {quiz.quizName}
                {quiz.topicName && <span className="opacity-70">· {quiz.topicName}</span>}
              </h3>
                  ...
                  <Card key={sim.id} className="overflow-hidden">
                    ...
                    >
                      <span className="flex min-w-0 items-start gap-3">
                        <Atom aria-hidden className="mt-0.5 size-4 shrink-0 text-primary" />
