 ...
 */
export function MaterialsBrowser({ materials }: { materials: StudentMaterial[] }) {
  ...
  const visible = useMemo(() => {
    ...
    return materials.filter((material) => {
      if (classId !== ALL_CLASSES && !material.classes.some((c) => c.id === classId)) return false;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        {classes.length > 1 && (
          <div className="flex flex-wrap gap-2">
            {[{ id: ALL_CLASSES, name: "All classes" }, ...classes].map((cls) => (
              <button
                key={cls.id}
                type="button"
              ...
              >
                ...
                <span className="min-w-0 flex-1 space-y-1">
                  <span className="flex flex-wrap items-center gap-2">
                    <span className="font-semibold">{displayName(material)}</span>
                    {material.topic && <Badge variant="secondary">{material.topic.name}</Badge>}
                  </span>
                  <span className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
                    <span>{material.classes.map((c) => c.name).join(", ")}</span>
                    <span>
                      {material.totalPages} page{material.totalPages !== 1 ? "s" : ""}
                    </span>
                    <span>{(material.sizeBytes / 1024 / 1024).toFixed(2)} MB</span>
