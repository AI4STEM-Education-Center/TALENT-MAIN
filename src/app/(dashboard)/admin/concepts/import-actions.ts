...
export async function uploadConceptsCsv(text: string): Promise<UploadResult> {
  try {
    ...
    return {
      ok: true,
      message:
        `Imported concepts — ${data.created} created, ${data.updated} updated, ${data.deleted} removed.` +
        (skipped.length > 0 ? ` ${skipped.length} row(s) skipped while parsing.` : ""),
      details: skipped.length > 0 ? skipped.map((s) => `Row ${s.row}: ${s.reason}`) : undefined,
    };
  } catch (error) {
...
export async function uploadMisconceptionsCsv(text: string): Promise<UploadResult> {
  try {
    ...
    return {
      ok: true,
      message:
        `Imported misconceptions — ${data.created} created, ${data.updated} updated, ${data.deleted} removed.` +
        (skipped.length > 0 ? ` ${skipped.length} row(s) skipped while parsing.` : ""),
      details: skipped.length > 0 ? skipped.map((s) => `Row ${s.row}: ${s.reason}`) : undefined,
    };
  } catch (error) {
