...
export function PressureTrendChart({ points }: { points: TrendPoint[] }) {
  const plotted = points
    .filter(
      (point): point is TrendPoint & { p95Ms: number } => point.p95Ms !== null,
    )
    .slice()
    .reverse();
  if (plotted.length === 0) {
    return (
      <div className="flex h-56 items-center justify-center text-sm text-muted-foreground">
        No latency samples match these filters.
      </div>
    );
  }
  ...
  const x = (index: number) =>
    PAD.left +
    (index / Math.max(1, plotted.length - 1)) * (WIDTH - PAD.left - PAD.right);
  const y = (value: number) =>
    PAD.top + (1 - value / max) * (HEIGHT - PAD.top - PAD.bottom);
  const path = plotted
    .map(
      (point, index) =>
        `${index === 0 ? "M" : "L"} ${x(index)} ${y(point.p95Ms)}`,
    )
    .join(" ");
  return (
        ...
        {plotted.map((point, index) => (
          <g key={point.id}>
            <circle
              ...
              r="5"
              className={
                point.status === "PASS" ? "fill-emerald-500" : "fill-red-500"
              }
            >
