...
export async function POST(req: NextRequest) {
  ...
  let body: { contentType?: unknown; contentId?: unknown; reviewerId?: unknown; topicId?: unknown };
  try {
    body = await req.json();
  } catch {
  ...
  const contentType = body.contentType === "QUIZ" || body.contentType === "MATERIAL"
    ? body.contentType
    : null;
  ...
  if (!contentType || !contentId || !reviewerId) {
    return NextResponse.json({ error: "Content and an administrator are required." }, { status: 400 });
  }
  ...
  if (topicId && !topic) return NextResponse.json({ error: "Global topic not found." }, { status: 404 });
  if (!teacher) return NextResponse.json({ error: "Teacher not found." }, { status: 404 });

  let contentName: string;
  if (contentType === "QUIZ") {
    const quiz = await prisma.quiz.findFirst({ where: { id: contentId, teacherId: actor.teacherId } });
  ...
  const duplicate = await prisma.poolSubmission.findFirst({
    where: {
      teacherId: actor.teacherId,
      status: "PENDING",
      ...(contentType === "QUIZ" ? { quizId: contentId } : { materialId: contentId }),
    },
  });
  if (duplicate) {
    return NextResponse.json({ error: "This item already has a pending review request." }, { status: 409 });
  }
  ...
  try {
    ...
    if (result.failed > 0) throw new Error(result.errors.join("; ") || "Email delivery failed");
