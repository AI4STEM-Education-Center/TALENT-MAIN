...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ids = Array.isArray(body.materialIds)
    ? Array.from(new Set(body.materialIds.filter((id): id is string => typeof id === "string")))
    : [];
  if (ids.length === 0) return NextResponse.json({ error: "materialIds is required" }, { status: 400 });
  ...
  if (poolMaterials.length !== ids.length) {
    return NextResponse.json({ error: "One or more pool materials were not found." }, { status: 404 });
  }

  const imported = (
    await Promise.all(
      poolMaterials.map((material) =>
        deepCopyLearningMaterial(material.id, { teacherId: teacher.id, classId })
      )
    )
