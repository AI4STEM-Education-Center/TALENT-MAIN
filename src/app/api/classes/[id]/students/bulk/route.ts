...
export async function POST(
  ...
  for (const s of students) {
    const orgDefinedId = String(s?.orgDefinedId ?? "")
      .replace(/^#/, "")
      .trim();
    ...
    if (!orgDefinedId || !firstName || !lastName || !isValidEmail(email)) {
      return NextResponse.json(
        {
          error:
            "Every row needs an 81 number, first name, last name, and a valid email.",
        },
        { status: 400 },
      );
    }
  ...
  const existing = await prisma.classStudentList.findMany({
    where: {
      classId: id,
      orgDefinedId: { in: cleaned.map((s) => s.orgDefinedId) },
    },
