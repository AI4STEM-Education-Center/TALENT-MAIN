...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  let body: { gradeColumnName?: unknown; pointsAwarded?: unknown; reviewerId?: unknown };
  try {
    body = await req.json();
  } catch {
  ...
  const gradeColumnName = typeof body.gradeColumnName === "string" ? body.gradeColumnName.trim() : "";
  ...
  if (!gradeColumnName || gradeColumnName.length > 200) {
    return NextResponse.json({ error: "A grade column name (1-200 characters) is required." }, { status: 400 });
  }
  if (!Number.isFinite(pointsAwarded) || pointsAwarded <= 0 || pointsAwarded > 1_000_000) {
    return NextResponse.json({ error: "Points awarded must be a number greater than 0." }, { status: 400 });
  }
  if (!reviewerId) return NextResponse.json({ error: "Choose an administrator to review this request." }, { status: 400 });

  const [reviewer, teacherUser] = await Promise.all([
    prisma.user.findFirst({ where: { id: reviewerId, role: "ADMIN" } }),
    prisma.user.findUnique({ where: { id: session.user.id }, select: { firstName: true, lastName: true, email: true } }),
  ]);
  if (!reviewer) return NextResponse.json({ error: "Administrator not found." }, { status: 404 });

  const pending = await prisma.consentExportRequest.findFirst({
    where: { classId: id, teacherId: owned.teacher.id, status: "PENDING" },
  });
  if (pending) {
    return NextResponse.json({ error: "This class already has a pending export request." }, { status: 409 });
  }
  ...
  try {
    ...
    console.error("[ConsentExportRequest] Failed to enqueue admin notification:", error);
  }
