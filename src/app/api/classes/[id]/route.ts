...
export async function DELETE(
  ...
  const linkedMaterials = await prisma.materialClass.findMany({
    ...
    select: {
      materialId: true,
      material: { select: { storageKey: true, bucket: true } },
    },
  });
  ...
  for (const link of linkedMaterials) {
    const remaining = await prisma.materialClass.count({
      where: { materialId: link.materialId },
    });
    ...
    if (bucket && link.material) {
      try {
        ...
        console.error(
          "Failed to delete S3 objects for material:",
          link.materialId,
          e,
        );
      }
    ...
    await prisma.learningMaterial
      .delete({ where: { id: link.materialId } })
      .catch(() => {});
  }
