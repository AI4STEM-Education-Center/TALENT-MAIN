...
describe("SIMULATION_TELEMETRY_SNIPPET", () => {
  it("is a single inline script that reports via postMessage only", () => {
    ...
    expect(SIMULATION_TELEMETRY_SNIPPET).toContain(
      `"${SIM_TELEMETRY_MESSAGE_TYPE}"`,
    );
  });
...
describe("clamping", () => {
  it("clamps durations and counts into their caps", () => {
    ...
    expect(clampTelemetryCount(MAX_TELEMETRY_COUNT * 2)).toBe(
      MAX_TELEMETRY_COUNT,
    );
  });
});
...
describe("sanitizeControlCounts", () => {
  ...
  it("truncates long labels and merges the collisions", () => {
    ...
    expect(sanitizeControlCounts({ [long]: 2, [truncated]: 3 })).toEqual({
      [truncated]: 5,
    });
  });
  ...
  it("caps the entry count, keeping the most-changed controls", () => {
    const input = Object.fromEntries(
      Array.from({ length: MAX_TELEMETRY_CONTROLS + 10 }, (_, i) => [
        `control-${i}`,
        i + 1,
      ]),
    );
    ...
    expect(kept[`control-${MAX_TELEMETRY_CONTROLS + 9}`]).toBe(
      MAX_TELEMETRY_CONTROLS + 10,
    );
...
describe("mergeControlCounts", () => {
  it("takes the per-key max (cumulative totals, not deltas)", () => {
    expect(
      mergeControlCounts({ mass: 5, angle: 2 }, { mass: 3, angle: 7, g: 1 }),
    ).toEqual({
      mass: 5,
      angle: 7,
      g: 1,
