...
function settings(overrides: Partial<GuardrailSettings> = {}): GuardrailSettings {
...
describe("GUARDRAIL_SURFACE_LABELS", () => {
  it("labels every registered surface", () => {
    ...
    expect(Object.keys(GUARDRAIL_SURFACE_LABELS)).toHaveLength(GUARDRAIL_SURFACES.length);
  });
});
...
describe("normalizeGuardrailSettings", () => {
  ...
  it("rejects an unknown mode rather than storing it", () => {
    const result = normalizeGuardrailSettings({ jailbreakMode: "block", offTopicMode: 7 });
    expect(result.jailbreakMode).toBe("FLAG");
    expect(result.offTopicMode).toBe("OFF");
  });

  it("clamps thresholds into 0–1", () => {
    const result = normalizeGuardrailSettings({ jailbreakThreshold: 9, offTopicThreshold: -4 });
  ...
  it("degrades a non-numeric threshold to the default rather than NaN", () => {
    expect(normalizeGuardrailSettings({ jailbreakThreshold: "high" }).jailbreakThreshold).toBe(0.7);
  });

  it("caps the topic description", () => {
    const result = normalizeGuardrailSettings({ topicDescription: "x".repeat(5_000) });
  ...
  it("stores surfaces in registry order regardless of submitted order", () => {
    ...
    expect(result.disabledSurfaces).toEqual(["assistant_chat", "question_import"]);
  });

  it("ignores a non-array disabledSurfaces", () => {
    expect(normalizeGuardrailSettings({ disabledSurfaces: "assistant_chat" }).disabledSurfaces)
      .toEqual([]);
  });
});
...
describe("policyFor", () => {
  it("passes the configured modes and thresholds through", () => {
    expect(
      policyFor(settings({ jailbreakMode: "BLOCK", jailbreakThreshold: 0.9 }), "assistant_chat")
    ).toEqual({
      jailbreakMode: "BLOCK",
      offTopicMode: "OFF",
  ...
  it("returns an INERT policy for a disabled surface, so no call is billed", () => {
    const policy = policyFor(
      settings({ jailbreakMode: "BLOCK", disabledSurfaces: ["assistant_chat"] }),
      "assistant_chat"
    );
  ...
  it("leaves other surfaces alone when one is disabled", () => {
    const config = settings({ jailbreakMode: "BLOCK", disabledSurfaces: ["assistant_chat"] });
    expect(policyFor(config, "question_import").jailbreakMode).toBe("BLOCK");
  });

  it("does not disable an unrecognised surface string", () => {
    const config = settings({ jailbreakMode: "FLAG", disabledSurfaces: ["assistant_chat"] });
...
describe("moderationEnabledFor", () => {
  it("is off when moderation is switched off globally", () => {
    expect(moderationEnabledFor(settings({ moderationEnabled: false }), "assistant_chat")).toBe(
      false
    );
  });
  ...
  it("is off for a disabled surface even when moderation is on", () => {
    expect(
      moderationEnabledFor(settings({ disabledSurfaces: ["material_page"] }), "material_page")
    ).toBe(false);
  });
