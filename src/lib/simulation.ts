...
- Desktop (width >= 700px): fit the COMPLETE simulation in the viewport, including every formula, symbol definition, control and readout, without page or nested scrollbars. Target a 900px by 600px frame. html/body are height:100%, margin:0. Use a three-row grid (compact header, auto-sized formula panel, minmax(0,1fr) main area). Wrap formula cards according to their intrinsic widths; never force wide equations into narrow fixed columns. Do not cap the formula panel or control panel with fixed heights, max-height, or overflow:auto. Use compact spacing and a two-column control/readout grid when height is limited. The main area is visual-left/control-right and receives most of the space. Give shrinking grid/flex children min-width:0 and min-height:0; never hide overflowing content to make the frame appear to fit.
