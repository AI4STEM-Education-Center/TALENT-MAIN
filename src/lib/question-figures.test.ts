...
describe("presignQuestionFigure", () => {
  it("returns null and never presigns when there is no figure key", async () => {
    expect(await presignQuestionFigure({ figureStorageKey: null, figureBucket: null })).toBeNull();
  ...
  it("presigns against the row's explicit bucket for one hour", async () => {
    ...
    const url = await presignQuestionFigure({ figureStorageKey: "k.png", figureBucket: "row-bucket" });
  ...
  it("falls back to the configured default bucket when the row has none", async () => {
    mockConfig.mockReturnValue({ bucket: "default-bucket", region: "us-east-1" });
    mockPresign.mockResolvedValue("https://signed.example/figure.png");
    await presignQuestionFigure({ figureStorageKey: "k.png", figureBucket: null });
  ...
  it("drops the figure (returns null) when presigning rejects", async () => {
    ...
    expect(await presignQuestionFigure({ figureStorageKey: "k.png", figureBucket: "b" })).toBeNull();
  });
  ...
  it("drops the figure when S3 config is missing for a bucketless row", async () => {
    ...
    expect(await presignQuestionFigure({ figureStorageKey: "k.png", figureBucket: null })).toBeNull();
  });
});
...
describe("attachFigureUrls", () => {
  it("swaps key/bucket for a presigned figureUrl and passes other fields through", async () => {
    ...
    const out = await attachFigureUrls([
      { id: "q1", text: "Question one", figureStorageKey: "k.png", figureBucket: "b" },
    ]);
    expect(out).toEqual([
      { id: "q1", text: "Question one", figureUrl: "https://signed.example/figure.png" },
    ]);
  ...
  it("sets figureUrl to null for figureless rows", async () => {
    const out = await attachFigureUrls([
      { id: "q2", text: "no figure", figureStorageKey: null, figureBucket: null },
    ]);
