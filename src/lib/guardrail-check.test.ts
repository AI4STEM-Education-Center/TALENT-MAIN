...
describe("buildGuardrailCheckPrompt", () => {
  ...
  it("does not let the text forge a closing fence", () => {
    ...
    expect(
      prompt.match(/\[END UNTRUSTED content under review\]/g),
    ).toHaveLength(1);
  });

  it("embeds the admin's topic description", () => {
    expect(
      buildGuardrailCheckPrompt("hi", "Only 19th century poetry"),
    ).toContain("Only 19th century poetry");
  });

  it("falls back to the default topic when the description is blank", () => {
    expect(buildGuardrailCheckPrompt("hi", "   ")).toContain(
      DEFAULT_TOPIC_DESCRIPTION,
    );
  });
  ...
  it("asks only the selected question, renumbered", () => {
    const jailbreakOnly = buildGuardrailCheckPrompt(
      "x",
      DEFAULT_TOPIC_DESCRIPTION,
      {
        jailbreak: true,
    ...
    const offTopicOnly = buildGuardrailCheckPrompt(
      "x",
      DEFAULT_TOPIC_DESCRIPTION,
      {
        jailbreak: false,
  ...
  it("only carries the topic description when off-topic is asked", () => {
    ...
    expect(
      buildGuardrailCheckPrompt("x", topic, {
        jailbreak: true,
        offTopic: false,
      }),
    ).not.toContain(topic);
...
describe("guardrailCheckSchema", () => {
  ...
  it("declares ONLY the checks the call asks about", () => {
    ...
    const jailbreakOnly = guardrailCheckSchema({
      jailbreak: true,
      offTopic: false,
    });
    ...
    const offTopicOnly = guardrailCheckSchema({
      jailbreak: false,
      offTopic: true,
    });
...
describe("activeChecks", () => {
  it("reads a mode of OFF as switched off and anything else as on", () => {
    expect(
      activeChecks(policy({ jailbreakMode: "FLAG", offTopicMode: "OFF" })),
    ).toEqual({
      jailbreak: true,
      offTopic: false,
    });
    expect(
      activeChecks(policy({ jailbreakMode: "BLOCK", offTopicMode: "BLOCK" })),
    ).toEqual({
      jailbreak: true,
      offTopic: true,
    });
...
describe("validateGuardrailCheck", () => {
  ...
  it("clamps confidence into 0–1", () => {
    const high = validateGuardrailCheck({
      jailbreak: { detected: true, confidence: 7 },
    });
    const low = validateGuardrailCheck({
      jailbreak: { detected: true, confidence: -3 },
    });
  ...
  it("degrades a non-numeric confidence to 0 rather than NaN", () => {
    const result = validateGuardrailCheck({
      jailbreak: { detected: true, confidence: "high" },
    });
  ...
  it("only treats a literal true as detected", () => {
    expect(
      validateGuardrailCheck({ jailbreak: { detected: "true" } }).jailbreak
        .detected,
    ).toBe(false);
  });
  ...
  it("drops a blank reason and caps a long one", () => {
    expect(
      validateGuardrailCheck({ jailbreak: { reason: "   " } }).jailbreak.reason,
    ).toBeNull();
    const long = validateGuardrailCheck({
      jailbreak: { reason: "z".repeat(1000) },
    });
...
describe("decideAction", () => {
  ...
  it("BLOCK blocks", () => {
    expect(
      decideAction(tripped, policy({ jailbreakMode: "BLOCK" })).blocked,
    ).toBe(true);
  });
  ...
  it("OFF ignores the finding entirely", () => {
    const action = decideAction(
      tripped,
      policy({ jailbreakMode: "OFF", offTopicMode: "OFF" }),
    );
    expect(action).toEqual({ blocked: false, reasons: [] });
  });

  it("does not trip below the threshold", () => {
    const weak = validateGuardrailCheck({
      jailbreak: { detected: true, confidence: 0.5 },
    });
    expect(
      decideAction(
        weak,
        policy({ jailbreakMode: "BLOCK", jailbreakThreshold: 0.7 }),
      ).reasons,
    ).toEqual([]);
  });

  it("trips exactly AT the threshold", () => {
    const exact = validateGuardrailCheck({
      jailbreak: { detected: true, confidence: 0.7 },
    });
    expect(
      decideAction(
        exact,
  ...
  it("ignores a high confidence when nothing was detected", () => {
    const undetected = validateGuardrailCheck({
      jailbreak: { detected: false, confidence: 0.99 },
    });
    expect(
      decideAction(undetected, policy({ jailbreakMode: "BLOCK" })).reasons,
    ).toEqual([]);
  });
  ...
  it("reports nothing for a clean verdict", () => {
    const action = decideAction(
      validateGuardrailCheck(CLEAN),
      policy({ offTopicMode: "BLOCK" }),
    );
    expect(action).toEqual({ blocked: false, reasons: [] });
  });
});
...
describe("policyIsInert", () => {
  it("is inert only when BOTH checks are off", () => {
    expect(
      policyIsInert(policy({ jailbreakMode: "OFF", offTopicMode: "OFF" })),
    ).toBe(true);
    expect(
      policyIsInert(policy({ jailbreakMode: "FLAG", offTopicMode: "OFF" })),
    ).toBe(false);
  });
