...
const inPrisma = (rel: string) =>
  `file:${path.join(process.cwd(), "prisma", rel)}`;
...
describe("resolveDatabaseUrl", () => {
  ...
  it("leaves an already-absolute file path untouched", () => {
    expect(resolveDatabaseUrl("file:/var/data/prod.db")).toBe(
      "file:/var/data/prod.db",
    );
  });

  it("re-anchors a relative path to <cwd>/prisma and makes it absolute", () => {
    expect(resolveDatabaseUrl("file:./data/prod.db")).toBe(
      inPrisma("./data/prod.db"),
    );
  ...
  it("preserves an absolute path while still stripping query params", () => {
    expect(resolveDatabaseUrl("file:/srv/app.db?connection_limit=1")).toBe(
      "file:/srv/app.db",
    );
  });
});
