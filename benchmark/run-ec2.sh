...
#
#   benchmark/ec2/provision.sh --source-instance i-0abc --ack-real-data
#   benchmark/run-ec2.sh --run-id bench-20260820-193000 --scenario exam-day
#   benchmark/ec2/teardown.sh --run-id bench-20260820-193000
#
...
for _ in $(seq 1 60); do
  if lg "test -f /opt/bench/READY" 2>/dev/null; then break; fi
  sleep 10
done
lg "test -f /opt/bench/READY" || die "the load generator never became ready; check 'sudo cat /var/log/bench/boot.log' on ${LOADGEN_IP}"
...
for _ in $(seq 1 90); do
  if sut "test -f /opt/bench/BOOTED" 2>/dev/null; then break; fi
  sleep 10
done
sut "test -f /opt/bench/BOOTED" || die "the clone never finished cloud-init; check 'sudo cat /var/log/bench/boot.log' via ${LOADGEN_IP}"

if sut "test -f /opt/bench/READY" 2>/dev/null; then
  ...
  tar -C "$BENCH_DIR" -czf - ec2/sanitize-sut.sh ec2/bootstrap-sut.sh ec2/docker-compose.sut.yml instrument/probe.cjs \
  ...
  sut "mkdir -p /tmp/payload && tar -C /tmp/payload -xzf /tmp/payload.tgz \
       && sudo install -m 0700 /tmp/payload/ec2/sanitize-sut.sh /opt/bench/sanitize-sut.sh \
       && sudo install -m 0700 /tmp/payload/ec2/bootstrap-sut.sh /opt/bench/bootstrap-sut.sh \
       && sudo install -m 0644 /tmp/payload/instrument/probe.cjs /opt/bench/probe.cjs \
  ...
  # BENCH_ACK_REAL_DATA is one of sanitize-sut.sh's three identity checks; it
  ...
  set +e
  sut "BENCH_SOURCE_INSTANCE_ID='$(jq -r '.sourceInstance' "$STATE_FILE")' \
       BENCH_ACK_REAL_DATA='yes' \
       BENCH_DEADMAN_MINUTES='$(jq -r '.deadmanMinutes // 240' "$STATE_FILE")' \
       /opt/bench/bootstrap-sut.sh"
  BOOTSTRAP_EXIT=$?
  set -e
  if [ $BOOTSTRAP_EXIT -ne 0 ]; then
    echo >&2
    echo "run-ec2: FATAL: bootstrap failed (exit ${BOOTSTRAP_EXIT}). Docker remains masked and the app is NOT running." >&2
    sut "sudo tail -40 /var/log/bench/sanitize.log" 2>/dev/null >&2 || true
    echo >&2
    echo "run-ec2: do NOT work around this. Terminate the clone:" >&2
    echo "run-ec2:   benchmark/ec2/teardown.sh --run-id ${RUN_ID} --region ${REGION}" >&2
    exit 1
  fi
fi
...
if ! sut "test -f /opt/bench/SANITIZED" 2>/dev/null; then
  echo >&2
...
sut "sudo cat /opt/bench/sanitize-report.json" | sed 's/^/    /'

sut "test -f /opt/bench/READY" || die "the clone sanitized but the application never became healthy; check 'sudo tail -50 /var/log/bench/boot.log'"
...
lg "rm -rf /opt/bench/harness && mkdir -p /opt/bench/harness"
tar -C "$REPO_DIR" -czf - benchmark package.json package-lock.json prisma src/lib/db-url.ts \
  | lg "tar -C /opt/bench/harness -xzf -"
...
lg "cd /opt/bench/harness && npm ci --no-audit --no-fund >/dev/null 2>&1 && npx prisma generate >/dev/null 2>&1" \
...
CLONE_SECRET="$(sut "sudo cat /opt/bench/auth-secret")"
...
lg "scp -o StrictHostKeyChecking=accept-new ubuntu@${SUT_IP}:/tmp/mint.db /opt/bench/mint.db" >/dev/null
sut "rm -f /tmp/mint.db"

log "minting sessions from the real user set..."
lg "cd /opt/bench/harness && AUTH_SECRET='${CLONE_SECRET}' npx tsx benchmark/tools/mint-sessions.ts \
      --out /opt/bench/sessions.json --database-url 'file:/opt/bench/mint.db' \
...
lg "tar -C /opt/bench/harness -czf - benchmark/mock-ai benchmark/tools \
    | ssh -o StrictHostKeyChecking=accept-new ubuntu@${SUT_IP} 'mkdir -p /opt/bench/ai && tar -C /opt/bench/ai -xzf -'"
...
sut "cd /opt/bench/ai && (nohup npx --yes tsx benchmark/mock-ai/server.ts --port 8099 --host 0.0.0.0 > /tmp/mock-ai.log 2>&1 &) ; sleep 3; curl -fsS http://127.0.0.1:8099/healthz || true"

RUN_DIR_REMOTE="/opt/bench/run"
...
lg "cd /opt/bench/harness && nohup benchmark/collect/metrics.sh \
...
EXTRA_ENV=""
[ -n "$COHORT" ] && EXTRA_ENV="BENCH_COHORT=${COHORT}"

set +e
lg "cd /opt/bench/harness && \
    BENCH_TIER=ec2-clone \
    BENCH_BASE_URL='http://${SUT_IP}:3000' \
    BENCH_SESSION_FILE=/opt/bench/sessions.json \
...
lg "cd /opt/bench/harness && benchmark/collect/metrics.sh --probe-once \
...
sut "sudo cat /opt/bench/sanitize-report.json" > /tmp/sanitize-report.json 2>/dev/null || true
...
lg "cd /opt/bench/harness && npx tsx benchmark/collect/scrub.ts --in ${RUN_DIR_REMOTE} --out /opt/bench/run-scrubbed" \
  || die "scrub failed — refusing to download unscrubbed artifacts from a clone holding production data"

LOCAL_RUN_DIR="${BENCH_DIR}/.tmp/ec2-runs/${RUN_ID}-${SCENARIO}"
mkdir -p "$LOCAL_RUN_DIR"
log "downloading scrubbed artifacts..."
scp "${SSH_OPTS[@]}" -r "ubuntu@${LOADGEN_IP}:/opt/bench/run-scrubbed/*" "$LOCAL_RUN_DIR/" >/dev/null
...
if [ "$SKIP_TEARDOWN" = "no" ]; then
  ...
  "${BENCH_DIR}/ec2/teardown.sh" --run-id "$RUN_ID" --region "$REGION"
else
  echo
  log "The clone is STILL RUNNING and still holds production data. When you are done:"
  log "  benchmark/ec2/teardown.sh --run-id ${RUN_ID} --region ${REGION}"
fi
