...
#   3. `teardown.sh --all`, which filters on the Purpose=alw-benchmark tag.
#
# Usage:
#   benchmark/ec2/provision.sh --source-instance i-0abc123 --ack-real-data \
...
provision: --ack-real-data is required.
  ...
  If you want realistic row counts without real records, seed a synthetic
  dataset instead: benchmark/seed/seed-bench.ts (see README, "Tier 3 with
  synthetic data").
...
cleanup_on_failure() {
  ...
  if [ "$KEEP_ON_FAILURE" = "yes" ]; then
    ...
    log "Clean up with: benchmark/ec2/teardown.sh --run-id ${RUN_ID} --region ${REGION}"
    return 0
  fi
  log "FAILED (exit ${code}) — tearing down everything this run created..."
  write_state
  "${SCRIPT_DIR}/teardown.sh" --run-id "$RUN_ID" --region "$REGION" || \
    log "WARNING: automatic teardown did not complete. RUN THIS: benchmark/ec2/teardown.sh --all --region ${REGION}"
}
...
CREATED_AMI="$(aws ec2 create-image --region "$REGION" \
  ...
  --no-reboot \
  --tag-specifications "ResourceType=image,Tags=[{Key=${TAG_KEY},Value=${TAG_VALUE}},{Key=BenchRunId,Value=${RUN_ID}}]" \
...
CREATED_SG_LOADGEN="$(aws ec2 create-security-group --region "$REGION" \
  ...
  --vpc-id "$SRC_VPC" \
  --tag-specifications "ResourceType=security-group,Tags=[{Key=${TAG_KEY},Value=${TAG_VALUE}},{Key=BenchRunId,Value=${RUN_ID}}]" \
...
CREATED_SUT="$(aws ec2 run-instances --region "$REGION" \
  ...
  --tag-specifications "ResourceType=instance,Tags=[{Key=${TAG_KEY},Value=${TAG_VALUE}},{Key=BenchRunId,Value=${RUN_ID}},{Key=Name,Value=${RUN_ID}-sut},{Key=BenchRole,Value=sut}]" \
...
INSTALL_K6_B64="$(base64 < "${BENCH_DIR}/tools/install-k6.sh" | tr -d '\n')"
...
CREATED_LOADGEN="$(aws ec2 run-instances --region "$REGION" \
  ...
  --tag-specifications "ResourceType=instance,Tags=[{Key=${TAG_KEY},Value=${TAG_VALUE}},{Key=BenchRunId,Value=${RUN_ID}},{Key=Name,Value=${RUN_ID}-loadgen},{Key=BenchRole,Value=loadgen}]" \
  --query 'Instances[0].InstanceId' --output text)"
write_state
...
sanitize wrote its success marker:
  ...
  benchmark/run-ec2.sh --run-id ${RUN_ID} --scenario exam-day

If you never run it, the clone sits with the app stopped until the deadman timer
terminates it. Watch the sanitize log with:

  ssh -A ubuntu@${LOADGEN_PUBLIC_IP} "ssh ubuntu@${SUT_PRIVATE_IP} 'sudo cat /var/log/bench/sanitize.log'"

WHEN YOU ARE DONE — this is not optional, these instances cost money:

  benchmark/ec2/teardown.sh --run-id ${RUN_ID} --region ${REGION}
