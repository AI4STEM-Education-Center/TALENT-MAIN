...
#                     effects, and writes /opt/bench/SANITIZED only if every
...
[ -f /opt/bench/sanitize-sut.sh ] || die "sanitize-sut.sh was not delivered to /opt/bench"
[ -f /opt/bench/probe.cjs ] || die "probe.cjs was not delivered to /opt/bench"
[ -f "${APP_DIR}/docker-compose.sut.yml" ] || die "docker-compose.sut.yml was not delivered to ${APP_DIR}"

log "running sanitize..."
chmod +x /opt/bench/sanitize-sut.sh
...
BENCH_SOURCE_INSTANCE_ID="${BENCH_SOURCE_INSTANCE_ID:-}" \
BENCH_ACK_REAL_DATA="${BENCH_ACK_REAL_DATA:-}" \
BENCH_DEADMAN_MINUTES="${BENCH_DEADMAN_MINUTES:-240}" \
BENCH_DB_PATH="${BENCH_DB_PATH:-}" \
  sudo -E /opt/bench/sanitize-sut.sh 2>&1 | sudo tee -a /var/log/bench/sanitize.log
...
[ -f /opt/bench/SANITIZED ] || die "sanitize did NOT complete — /opt/bench/SANITIZED is absent. Docker stays masked. Terminate this clone."

log "sanitize confirmed. Unmasking Docker..."
sudo rm -f /opt/bench/AWAITING_SANITIZE
