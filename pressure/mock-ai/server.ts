 ...
 *
 * Usage:
 *   tsx pressure/mock-ai/server.ts --port 8099 --latency-ms 400
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === "GET" && (req.url ?? "").endsWith("/models")) {
      ...
      res.end(
        JSON.stringify({
          object: "list",
          data: [{ id: "pressure-stub-1", object: "model", owned_by: "benchmark" }],
        })
      );
      return;
    }
    ...
    req.on("end", () => {
      ...
      const model = body.model ?? "pressure-stub-1";
