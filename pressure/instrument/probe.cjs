 ...
 * `NODE_OPTIONS=--require /app/pressure-probe.cjs`.
 *
...
const PORT = Number(process.env.PRESSURE_PROBE_PORT || 9099);
const HOST = process.env.PRESSURE_PROBE_HOST || "127.0.0.1";
...
const RESOLUTION_MS = Number(process.env.PRESSURE_PROBE_RESOLUTION_MS || 10);
...
function snapshot() {
  ...
  return {
    ...
    node: {
      ...
      // moment PRESSURE_PROBE_ROLE was set at all — which made the report attribute
      ...
      // started without it.
      role:
        process.env.PRESSURE_PROBE_ROLE ||
