...
#     IS production.
if [ -n "${PRESSURE_SOURCE_INSTANCE_ID:-}" ] && [ "$INSTANCE_ID" = "$PRESSURE_SOURCE_INSTANCE_ID" ]; then
...
if [ "$PURPOSE" != "talent-pressure" ]; then
  die "instance $INSTANCE_ID is not tagged Purpose=talent-pressure (got '${PURPOSE:-<none>}'). Refusing to touch it."
fi
...
[ "${PRESSURE_ACK_REAL_DATA:-}" = "yes" ] || \
  die "PRESSURE_ACK_REAL_DATA=yes was not passed. The operator must acknowledge that this clone holds production data."

log "identity confirmed: clone $INSTANCE_ID (source was ${PRESSURE_SOURCE_INSTANCE_ID:-unknown})"
...
DB_PATH="${PRESSURE_DB_PATH:-${APP_DIR}/data/db/prod/prod.db}"
...
for compose in docker-compose.caddy.yml docker-compose.yml docker-compose.dev.yml; do
  if [ -f "${APP_DIR}/${compose}" ]; then
    mv "${APP_DIR}/${compose}" "${APP_DIR}/${compose}.disabled-for-pressure"
...
DEADMAN_MIN="${PRESSURE_DEADMAN_MINUTES:-240}"
...
cat > "$REPORT" <<JSON
{
  ...
  "sourceInstanceId": "${PRESSURE_SOURCE_INSTANCE_ID:-unknown}",
  "controls": {
