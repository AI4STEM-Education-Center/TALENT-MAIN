...
// or the exported PRESSURE_* vars the runners set). `open()` IS available, but only
...
export const RUN_LABEL = __ENV.PRESSURE_RUN_LABEL || "unlabelled";

/**
 * Refuse to run a scenario outside the tier it is valid for.
 *
 * This is the most important safety property of the harness: a scenario cannot
 * be pointed at the shared live dev/production host by changing a URL alone.
 */
...
/**
 * Hard ceiling on VUs, applied to the computed shape rather than trusted from
 * PRESSURE_SCALE.
 */
...
const SESSION_FILE = __ENV.PRESSURE_SESSION_FILE || "";
...
export const COOKIE_NAME = __ENV.PRESSURE_COOKIE_NAME || "authjs.session-token";
 ...
 */
export const FORWARDED_HOST = __ENV.PRESSURE_FORWARDED_HOST || "";
 ...
 *
 * Optional explicit class+quiz target for media-signing investigations.
 * `{}` when unset, which makes callers discover a suitable live-clone quiz.
 */
...
export const CLOUDFRONT_EXPECTED = __ENV.PRESSURE_EXPECT_CLOUDFRONT === "1";
 ...
 *
 * Override with PRESSURE_FETCH_MEDIA=1 / =0 when a local run does have a real CDN.
 */
export const FETCH_MEDIA =
  __ENV.PRESSURE_FETCH_MEDIA !== undefined && __ENV.PRESSURE_FETCH_MEDIA !== ""
    ? __ENV.PRESSURE_FETCH_MEDIA === "1"
    : true;
