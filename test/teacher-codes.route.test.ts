...
describe("/api/admin/teacher-codes", () => {
  it("refuses every verb to a non-admin", async () => {
    ...
    expect(
      (await PATCH(patchReq("x", { active: false }), params("x"))).status,
    ).toBe(403);
    expect(
      (
        await DELETE(
  ...
  it("issues a code with no duration and no use limit", async () => {
    ...
    expect(body.url).toMatch(
      new RegExp(
        `^https?://[^/]+/register\\?code=${normalizeTeacherCode(body.code)}$`,
      ),
    );
  ...
  it("rejects a duration or use limit outside its bounds, naming the bound", async () => {
    ...
    const tooLong = await POST(
      createReq({ expiresInMinutes: 6 * 365 * 24 * 60 }),
    );
  ...
  it("rejects a non-integer duration as a shape error", async () => {
    expect((await POST(createReq({ expiresInMinutes: 10.5 }))).status).toBe(
      400,
    );
  ...
  it("lists codes newest-first with their computed status", async () => {
    const first = await (
      await POST(createReq({ label: "first", maxUses: 1 }))
    ).json();
    ...
    expect(body.codes.map((c: { id: string }) => c.id)).toEqual([
      second.id,
      first.id,
    ]);
  ...
  it("revokes and restores a code without touching its usage count", async () => {
    ...
    const revoked = await (
      await PATCH(patchReq(created.id, { active: false }), params(created.id))
    ).json();
    ...
    const restored = await (
      await PATCH(patchReq(created.id, { active: true }), params(created.id))
    ).json();
  ...
  it("404s when revoking or deleting a code that does not exist", async () => {
    expect(
      (await PATCH(patchReq("nope", { active: false }), params("nope"))).status,
    ).toBe(404);
    const del = await DELETE(
      new NextRequest(`${BASE}/nope`, { method: "DELETE" }),
      params("nope"),
    );
