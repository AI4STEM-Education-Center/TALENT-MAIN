...
const bucketContents = new Map<string, { body: Uint8Array; contentType: string }>();
...
vi.mock("@/lib/storage", async (importOriginal) => {
  ...
  return {
    ...actual,
    getS3Config: vi.fn(() => {
      if (!configuredBucket) throw new Error("Learning materials require AWS_S3_BUCKET");
      return { bucket: configuredBucket, region: "us-east-1" };
    }),
    putS3Object: vi.fn(async (_bucket: string, key: string, body: Uint8Array, contentType: string) => {
    ...
    signObjectReadUrl: vi.fn(async (_bucket: string, key: string) => `https://cdn.test/${key}`),
  };
});
...
const request = (id: string) =>
  GET_ATTACHMENT(new Request(`http://localhost/api/assistant/attachments/${id}`), {
...
describe("persistAttachments", () => {
  it("stores the bytes and indexes them with the configured expiry", async () => {
    ...
    const days = (new Date(stored[0].expiresAt).getTime() - before) / 86_400_000;
    ...
    expect(row).toMatchObject({ userId: user.id, audience: "student", bytes: 5 });
    expect(bucketContents.get(row.storageKey)?.body).toEqual(Buffer.from("hello"));
  });
  ...
  it("keys the object by user and attachment id, so same-named uploads can't collide", async () => {
    ...
    const stored = await persistAttachments(
      { userId: user.id, audience: "student" },
      [image("a.png", HELLO), image("a.png", Buffer.from("second").toString("base64"))],
      30
    );
    expect(stored).toHaveLength(2);
    const rows = await prisma.assistantAttachment.findMany({ select: { storageKey: true } });
  ...
  it("does nothing, and does not throw, when there is no object storage", async () => {
    ...
    expect(
      await persistAttachments({ userId: user.id, audience: "student" }, [image()], 30)
    ).toEqual([]);
...
describe("loadStoredAttachments", () => {
  it("reads the bytes back for the owner, in the order asked for", async () => {
    ...
    const stored = await persistAttachments(
      { userId: user.id, audience: "student" },
      [image("one.png", HELLO), image("two.png", Buffer.from("world").toString("base64"))],
      30
    );
    ...
    expect(Buffer.from(loaded[0].dataBase64, "base64").toString()).toBe("world");
  ...
  it("returns nothing for another user's attachment id", async () => {
    ...
    expect(await loadStoredAttachments(mine.user.id, [stored[0].id], 4)).toEqual([]);
  });
  ...
  it("skips an id whose object has vanished rather than failing the turn", async () => {
    ...
    const row = await prisma.assistantAttachment.findFirstOrThrow({ where: { name: "gone.png" } });
...
describe("POST /api/assistant/chat — persisting a turn's attachments", () => {
  ...
  async function chat(body: unknown): Promise<AssistantStreamEvent[]> {
    ...
    for await (const event of readNdjson<AssistantStreamEvent>(res.body!)) events.push(event);
    return events;
  }
  ...
  it("stores the attachment and streams its id before answering", async () => {
    ...
    const events = await chat({
      ...
      attachments: [{ name: "shot.png", mimeType: "image/png", dataBase64: HELLO }],
    });

    const announced = events.find((event) => event.type === "attachments");
    expect(announced).toBeDefined();
    const stored = (announced as { stored: Array<{ id: string; name: string }> }).stored;
    expect(stored).toHaveLength(1);
  ...
  it("does not store an attachment the admin's limits rejected", async () => {
    ...
    await saveAssistantSettings("student", { enabled: true, attachmentKinds: ["image"] });
    asStudent(user.id);

    const events = await chat({
      message: "read this",
      attachments: [{ name: "roster.csv", mimeType: "text/csv", dataBase64: HELLO }],
    });
    expect(events.some((event) => event.type === "attachments")).toBe(false);
    expect(await prisma.assistantAttachment.count()).toBe(0);
  });
