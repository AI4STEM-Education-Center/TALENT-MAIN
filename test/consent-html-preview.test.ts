...
describe("previewConsentHtml", () => {
  ...
  it("removes unsafe link schemes but keeps allowed ones", () => {
    expect(previewConsentHtml('<a href="javascript:alert(1)">click</a>')).toBe(
      "<a>click</a>",
    );
  ...
  it("unwraps disallowed containers instead of dropping their text", () => {
    ...
    expect(
      previewConsentHtml("<table><tr><td>Cell</td></tr></table>"),
    ).toContain("Cell");
  });
  ...
  it("agrees with the server sanitizer on the text this build ships", () => {
    ...
    for (const role of ["STUDENT", "TEACHER"] as const) {
      ...
      expect(normalize(previewConsentHtml(body))).toBe(
        normalize(sanitizeConsentHtml(body)),
      );
    }
  });
});
