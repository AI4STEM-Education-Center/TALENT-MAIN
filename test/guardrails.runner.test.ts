...
const recordGuardrailEvent = vi.fn(
  async (..._a: unknown[]): Promise<string | null> => "evt-1",
);
...
vi.mock("@/lib/guardrail-settings", async () => {
  const actual = await vi.importActual<
    typeof import("@/lib/guardrail-settings")
  >("@/lib/guardrail-settings");
  return { ...actual, getGuardrailSettings: () => getGuardrailSettings() };
});

const { guardText, guardChatTurn, auditText } =
  await import("@/lib/guardrail-runner");

function withSettings(overrides: Partial<GuardrailSettings>) {
  getGuardrailSettings.mockResolvedValue({
    ...defaultGuardrailSettings(),
    ...overrides,
  });
}
...
describe("guardText", () => {
  ...
  it("blocks on a safety BLOCK", async () => {
    ...
    expect(
      (await guardText("...", { surface: "question_authoring" })).blocked,
    ).toBe(true);
  });
  ...
  it("skips moderation for a disabled surface and passes an inert policy", async () => {
    ...
    expect(moderateText).not.toHaveBeenCalled();
    const options = checkContentSafety.mock.calls[0][2] as {
      policy: { jailbreakMode: string };
    };
...
describe("guardChatTurn", () => {
  ...
  it("blocks a flagged turn", async () => {
    ...
    expect(
      (await guardChatTurn("x", "x", { surface: "assistant_chat" })).blocked,
    ).toBe(true);
  });
...
describe("auditText", () => {
  it("reports findings but NEVER blocks", async () => {
    ...
    const decision = await auditText("...", {
  ...
  it("does nothing at all when the surface is fully disabled", async () => {
    withSettings({
      disabledSurfaces: ["material_description"],
      moderationEnabled: false,
    });
