...
describe("global-pool approval requests", () => {
  it("lets a teacher choose an admin and emails a direct review link", async () => {
    ...
    const quiz = await prisma.quiz.create({
      data: { name: "For review", teacherId: teacher.id },
    });
    ...
    expect(
      await prisma.poolSubmission.count({
        where: { quizId: quiz.id, status: "PENDING" },
      }),
    ).toBe(1);
    expect(mockSendEmail).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        text: expect.stringContaining(
          `/admin/pool-submissions?request=${submission.id}`,
        ),
      }),
    );
  });
  ...
  it("rejects a non-admin reviewer and duplicate pending requests", async () => {
    ...
    const quiz = await prisma.quiz.create({
      data: { name: "For review", teacherId: owner.teacher.id },
    });
    mockAuth.mockResolvedValue({
      user: { id: owner.user.id, role: "TEACHER" },
    } as never);
      ...
      (
        await SUBMIT(
          request({
            contentType: "QUIZ",
            contentId: quiz.id,
            reviewerId: otherTeacher.user.id,
          }),
        )
      ).status,
    ).toBe(404);
  ...
  it("allows only the selected admin to approve and publishes an independent quiz copy", async () => {
    ...
    const topic = await prisma.topic.create({
      data: { name: "Physics", teacherId: null },
    });
    const quiz = await prisma.quiz.create({
      data: { name: "For review", teacherId: owner.teacher.id },
    });
    await prisma.question.create({
      data: { text: "Question", quizId: quiz.id },
    });
    ...
    expect(
      (await DECIDE(request({ decision: "APPROVE" }), params(submission.id)))
        .status,
    ).toBe(404);

    mockAuth.mockResolvedValue({
      user: { id: reviewer.id, role: "ADMIN" },
    } as never);
    const response = await DECIDE(
      request({ decision: "APPROVE", note: "Looks good" }),
      params(submission.id),
    );
    expect(response.status).toBe(200);
    const poolCopy = await prisma.quiz.findFirst({
      where: { teacherId: null, sourceQuizId: quiz.id },
    });
    expect(poolCopy).toMatchObject({ name: "For review", topicId: topic.id });
    expect(
      await prisma.question.count({ where: { quizId: poolCopy!.id } }),
    ).toBe(1);
    expect(
      await prisma.poolSubmission.findUnique({ where: { id: submission.id } }),
    ).toMatchObject({
      status: "APPROVED",
